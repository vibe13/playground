...
:icons: font
////
icon:info-circle[1x,role=blue]
icon:exclamation-circle[1x,role=yellow]
icon:warning[1x,role=yellow]
icon:check-circle[1x,role=green]
icon:times-circle[1x,role=red]
////
:toc:
...
{ProductName} operates as a Build Platform and does not control repository governance.  
Source Track requirements are therefore outside scope of this evaluation.
...
* ✅ icon:check-circle[1x,role=green] MUST be generated by the build process.
* icon:check-circle[1x,role=green] MUST be formatted per SLSA guidelines, or contain equivalent information.
* icon:check-circle[1x,role=green] SHOULD be as complete as possible.
...
* icon:times-circle[1x,role=red] MUST be verifiable by users for authenticity.
* icon:check-circle[1x,role=green] MUST be generated by the control plane and not by a tenant of the build platform (unless the data is obtained from the build platform).
* icon:check-circle[1x,role=green] SHOULD be complete, although some external parameters MAY not be captured.
...
icon:info-circle[1x,role=blue] At the time of writing, it's not determined if the Orchestrator Service will actually generate the provenance, or provide all the required data to a delegated Service for the generation. This is however inline with the Level 2 requirements: "The data in the provenance MUST be obtained from the build platform, either because the generator is the build platform or because the provenance generator reads the data directly from the build platform".
...
* icon:check-circle[1x,role=green] Secret material used to authenticate provenance MUST be stored in a secure management system.
* icon:check-circle[1x,role=green] Secret material is not accessible to the environment running user-defined build steps.
* icon:times-circle[1x,role=red] Provenance is complete, including fully enumerated external parameters.
...
|L1
|icon:check-circle[1x,role=green] No build isolation requirements apply for L1 conformance.
|N/A

|L2: Hosted build platform
|icon:check-circle[1x,role=green] All build steps run on a hosted build platform using shared or dedicated infrastructure, not on an individual’s workstation.
...
* icon:check-circle[1x,role=green] It MUST NOT be possible for a build to access any build platform secrets, such as the provenance signing key.
* icon:check-circle[1x,role=green] It MUST NOT be possible for overlapping builds to influence one another.
* icon:check-circle[1x,role=green] It MUST NOT be possible for a build to persist or influence the environment of subsequent builds.
* icon:check-circle[1x,role=green] It MUST NOT be possible for a build to inject false entries into a cache used by another build (“cache poisoning”).
* icon:times-circle[1x,role=red] The build platform MUST NOT expose services allowing remote influence unless all such interactions are captured as `externalParameters` in the provenance.
